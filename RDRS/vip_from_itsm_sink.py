...
def get_config_info_0(link_info):
    ...
    try:
        ...
        vip = str(database_hostname.split(':')[0]).strip()
        link_info['vip'] = vip
...
def get_vip_info_0(link_info):
    ...
    try:
        ...
        link_info['vip_info'] = [{'vip': vip, 'err_msg': e}]
        traceback.print_exc()
...
def save_database(link_list):
    ...
    # 执行SQL查询
    truncate_sql = "TRUNCATE TABLE vip_from_itsm_sink;"
    cursor.execute(truncate_sql)

    insert_sql = "INSERT INTO vip_from_itsm_sink (vip, ip, role, sys_en, sys_cn, bk_ip, err_msg) VALUES (%s, %s, %s, %s, %s, %s, %s)"
    for link_dict in link_list:
        vip_info = link_dict.get('vip_info')
        if vip_info is None:
            continue
